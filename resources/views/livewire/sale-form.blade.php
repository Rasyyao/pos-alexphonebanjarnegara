<div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" wire:model.live="saleDate"
                            class="field-input @error('saleDate') error @enderror" style="width:100%" />
                        @error('saleDate')
                            <p class="field-error">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label class="field-label">Nama Pembeli <span class="text-[10px] font-normal"
                                style="color:var(--ink-mute)">(opsional)</span></label>
                        <input type="text" wire:model.live="customerName" placeholder="Nama pelanggan..."
                            class="field-input" style="width:100%" />
                    </div>
                </div>
                <div class="mt-4">
                    <label class="field-label">Keterangan <span class="text-[10px] font-normal"
                            style="color:var(--ink-mute)">(opsional)</span></label>
                    <textarea wire:model.live="description" rows="2" placeholder="Catatan atau keterangan tambahan transaksi..."
                        ...
                        <div class="p-5" style="{{ $i > 0 ? 'border-top:1px solid var(--line)' : '' }}">
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-3">
                                    <span
                                        class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold font-mono flex-shrink-0"
                                    ...
                                    <div class="flex rounded-lg overflow-hidden" style="border:1px solid var(--line)">
                                        <button wire:click="$set('items.{{ $i }}.type', 'unit')"
                                            type="button" class="px-3 py-1.5 text-xs font-medium transition-colors"
                                            style="{{ ($item['type'] ?? 'unit') === 'unit' ? 'background:var(--accent);color:#fff' : 'background:transparent;color:var(--ink-soft)' }}">
                                            HP Unit
                                        </button>
                                        <button wire:click="$set('items.{{ $i }}.type', 'accessory')"
                                            type="button" class="px-3 py-1.5 text-xs font-medium transition-colors"
                            ...
                            @if (($item['type'] ?? 'unit') === 'unit')
                                <div class="mb-4">
                                    ...
                                    <input type="hidden" wire:model.live="items.{{ $i }}.unit_id"
                                        id="ss-val-unit-{{ $i }}" />
                                    <div wire:ignore class="ss-wrap" data-target="ss-val-unit-{{ $i }}"
                                        data-placeholder="Pilih Unit HP (status ready)">
                                        <div class="ss-opts" style="display:none">
                                            @foreach ($readyUnits as $unit)
                                                <span data-v="{{ $unit->id }}">{{ $unit->model->brand->name }}
                                                    {{ $unit->model->name }} —
                                                    {{ $unit->ram }}/{{ $unit->rom }} {{ $unit->color }}
                                                    ({{ ucfirst($unit->unit_type->value) }}{{ $unit->grade ? ' - Grade ' . $unit->grade : '' }})</span>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-3 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" wire:model.live="items.{{ $i }}.quantity"
                                            min="1" placeholder="1" class="field-input" />
                                    </div>
                                </div>
                            @endif
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    <div class="flex rounded-lg overflow-hidden" style="border:1px solid var(--line)">
                                        @foreach (['cash' => 'Cash', 'transfer' => 'Transfer', 'utang' => 'Utang'] as $val => $label)
                                            <button
                                                wire:click="$set('payments.{{ $i }}.method', '{{ $val }}')"
                                                type="button"
                            ...
                            @if (($payment['method'] ?? 'cash') === 'utang')
                                <div class="mt-3 pt-3" style="border-top:1px dashed var(--line)">
                                    <div class="flex items-center gap-3 px-4 py-3 rounded-lg"
                                        ...
                                        <svg class="w-4 h-4 flex-shrink-0" style="color:var(--warn)" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <label class="text-xs font-semibold whitespace-nowrap flex-shrink-0"
                                            style="color:var(--warn)">Nama Pengutang</label>
                                        <input type="text" wire:model.live="customerName"
                                            placeholder="Isi nama pelanggan yang berutang..."
                                            class="field-input flex-1" style="font-size:13px;height:36px" />
                                        @if (trim($customerName))
                                            <span class="text-xs font-semibold whitespace-nowrap flex-shrink-0"
                                                style="color:var(--success)">✓ {{ $customerName }}</span>
                                        @else
                                            <span class="text-[11px] whitespace-nowrap flex-shrink-0"
                                                style="color:var(--warn)">Wajib diisi</span>
                                        @endif
                                    </div>
                                </div>
                            @endif
            ...
            <div class="bg-white rounded-xl border overflow-hidden sticky top-20" style="border-color:var(--line)">
                ...
                <div class="p-5 space-y-3">
                    ...
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-semibold font-mono tabular-nums" style="color:var(--ink)">Rp
                    ...
                    @foreach ($payments as $p)
                        @if (($p['amount'] ?? 0) > 0)
                            <div class="flex justify-between items-center text-xs" style="color:var(--ink-mute)">
                                ...
                                <span class="font-mono tabular-nums">Rp
                    ...
                    <div class="flex justify-between items-center text-sm pt-1"
                        ...
                        <span class="font-semibold font-mono tabular-nums" style="color:var(--ink)">Rp
                            {{ number_format(collect($payments)->sum('amount'), 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="pt-2" style="border-top:1px solid var(--line)">
                        @if ($remainder > 0)
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-sm font-bold font-mono tabular-nums" style="color:var(--warn)">Rp
                                    {{ number_format($remainder, 0, ',', '.') }}</span>
                            </div>
                        ...
                        @elseif($remainder < 0)
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-sm font-bold font-mono tabular-nums" style="color:var(--success)">Rp
                                    {{ number_format(abs($remainder), 0, ',', '.') }}</span>
                            </div>
                        @else
                ...
                <div class="px-5 pb-5">
                    ...
                    @if ($remainder > 0)
                        <p class="text-center text-xs mt-2" style="color:var(--warn)">Lengkapi pembayaran untuk
                            melanjutkan</p>
                    @endif
                </div>
            </div>
    ...
    @if ($savedSaleId)
        <div id="sale-success-modal"
            ...
            {{-- Modal card --}}
            <div
                style="position:relative;background:#fff;border-radius:20px;max-width:420px;width:100%;overflow:hidden;box-shadow:0 24px 64px rgba(10,37,64,.25);animation:modal-in 220ms cubic-bezier(.34,1.56,.64,1)">

                {{-- Green header --}}
                <div style="background:var(--success);padding:32px 28px 24px;text-align:center">
                    <div
                        style="width:60px;height:60px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 14px">
                        <svg style="width:30px;height:30px;color:#fff" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                    ...
                    <p
                        style="color:rgba(255,255,255,.8);font-size:12px;margin:0;font-family:var(--font-mono);letter-spacing:.04em">
                        {{ $savedInvoice }}</p>
                </div>
                ...
                <div style="padding:24px 28px 28px">
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px">
                        ...
                        <a href="{{ route('sales.print', $savedSaleId) }}" target="_blank" rel="noopener"
                            ...
                            <svg style="width:17px;height:17px" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
