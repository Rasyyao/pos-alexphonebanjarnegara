...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-xs mt-0.5" style="color:var(--ink-mute)">Analisis profitabilitas, arus kas operasional, dan pengelolaan modal usaha</p>
        </div>
    ...
    <div class="bg-white rounded-xl border p-4 shadow-sm" style="border-color:var(--line)">
        <form method="GET" action="{{ route('reports.finance') }}" id="date-filter-form" class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2 px-3 rounded-lg border bg-[#F8FAFC]" style="border-color:var(--line); height: 36px;">
            ...
            <div class="flex items-center gap-2 shrink-0 self-end md:self-auto">
                <button type="submit" class="text-xs h-9 px-4 font-semibold rounded-lg transition-colors flex items-center gap-1.5 shadow-sm"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        {{-- HARI INI (All Roles) --}}
        <div class="bg-white rounded-xl border p-5 shadow-sm hover:shadow transition-shadow" style="border-color:var(--line)">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-bold uppercase tracking-widest font-mono" style="color:var(--ink-mute)">Hari Ini (Daily)</span>
            ...
            <div class="space-y-2.5">
                ...
                <div class="pt-2 border-t" style="border-color:var(--line)">
                    <span class="text-[11px] font-medium" style="color:var(--ink-soft)">Laba Kotor</span>
        ...
        <div class="bg-white rounded-xl border p-5 shadow-sm hover:shadow transition-shadow" style="border-color:var(--line)">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-bold uppercase tracking-widest font-mono" style="color:var(--ink-mute)">Minggu Ini (Weekly)</span>
        ...
        @else
        <div class="bg-white rounded-xl border p-5 shadow-sm relative overflow-hidden flex flex-col justify-center border-dashed" style="border-color:var(--line); min-height: 150px;">
            <div class="absolute inset-0 bg-white/70 backdrop-blur-[1px] flex flex-col items-center justify-center text-center p-4">
                <svg class="w-5 h-5 mb-1.5" style="color:var(--ink-mute)" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                </svg>
                <span class="text-[11px] font-semibold" style="color:var(--ink-soft)">Minggu Ini (Weekly)</span>
                <span class="text-[9px] uppercase tracking-wider font-mono font-bold mt-0.5" style="color:var(--accent)">Superadmin Only</span>
            </div>
        </div>
        @endif
        ...
        <div class="bg-white rounded-xl border p-5 shadow-sm hover:shadow transition-shadow" style="border-color:var(--line)">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-bold uppercase tracking-widest font-mono" style="color:var(--ink-mute)">Bulan Ini (Monthly)</span>
    ...
    <div class="space-y-6 mt-6">
        ...
        <div class="bg-white rounded-xl border overflow-hidden shadow-sm" style="border-color:var(--line)">
            <div class="px-5 py-4 border-b flex items-center justify-between" style="border-color:var(--line)">
                <div>
                    <h3 class="text-sm font-semibold" style="color:var(--ink)">Biaya Pengeluaran Operasional (Gas, Listrik, Dll)</h3>
                    <p class="text-[11px] mt-0.5" style="color:var(--ink-mute)">Log biaya pengeluaran operasional toko yang mengurangi modal sekarang</p>
                </div>
                ...
                <table class="w-full text-xs">
                    <thead>
                        <tr style="background:var(--bg-soft); border-bottom:1px solid var(--line)">
                            <th class="text-left px-5 py-2.5 font-bold uppercase tracking-wider font-mono" style="color:var(--ink-mute)">Tanggal</th>
                            ...
                            <th class="text-left px-5 py-2.5 font-bold uppercase tracking-wider font-mono" style="color:var(--ink-mute)">Catatan</th>
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid var(--line)">
                            <td class="px-5 py-3 font-mono" style="color:var(--ink-soft)">{{ $expense->expense_date->format('d/m/Y') }}</td>
                            <td class="px-5 py-3 font-medium" style="color:var(--ink)">{{ $expense->description }}</td>
                            <td class="px-5 py-3">
                                <span class="px-2 py-0.5 rounded bg-gray-100 text-gray-700 font-mono text-[9px] capitalize">{{ $expense->category }}</span>
                            </td>
                            <td class="px-5 py-3" style="color:var(--ink-mute)">{{ $expense->notes ?: '—' }}</td>
                            <td class="px-5 py-3 text-right font-mono font-bold tabular-nums" style="color:var(--warn)">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-xs" style="color:var(--ink-mute)">Belum ada data pengeluaran dicatat</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="px-5 py-3 flex items-center justify-between" style="border-top:1px solid var(--line);background:var(--bg-soft)">
                <span class="text-xs font-mono" style="color:var(--ink-mute)">{{ $expenses->total() }} pengeluaran</span>
    ...
    <div id="modal-pengeluaran-quick" class="fixed inset-0 z-[100] hidden overflow-y-auto" onclick="closeExpenseModalOutside(event)">
            ...
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden modal-pop" onclick="event.stopPropagation()">
                ...
                <div class="flex items-start justify-between px-6 py-5" style="border-bottom:1px solid var(--line)">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs mt-1.5" style="color:var(--ink-mute)">Catat pengeluaran listrik, gas, gaji, dan biaya operasional</p>
                        </div>
                    </div>
                    <button onclick="closeExpenseModal()" class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors" style="color:var(--ink-mute);background:var(--bg-soft)" onmouseenter="this.style.background='var(--line)'" onmouseleave="this.style.background='var(--bg-soft)'">
                ...
                <form method="POST" action="{{ route('expenses.store') }}" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" placeholder="mis. Bayar listrik, Isi ulang tabung gas, Gaji admin..." required class="field-input" />
                    </div>
                    <div>
                      ...
                      <div class="flex gap-3 pt-1">
                          ...
                          <button type="button" onclick="closeExpenseModal()" class="btn-secondary" style="padding:0 24px">Batal</button>
                      </div>
                  </form>
