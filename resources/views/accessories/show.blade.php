...
@section('content')
    <div class="w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('accessories.edit', $accessory) }}" class="btn-primary"
                ...
                <div class="bg-white rounded-xl border overflow-hidden" style="border-color:var(--line)">
                    ...
                    <div class="divide-y">
                        <div class="flex items-center px-5 py-3" style="border-color:var(--line)">
                            <span class="w-36 text-xs font-medium flex-shrink-0" style="color:var(--ink-mute)">Nama
                                Aksesoris</span>
                            <span class="text-sm font-medium" style="color:var(--ink)">{{ $accessory->name }}</span>
                        </div>
                        <div class="flex items-center px-5 py-3" style="border-color:var(--line)">
                            <span class="w-36 text-xs font-medium flex-shrink-0"
                                style="color:var(--ink-mute)">Kategori</span>
                            <span class="text-sm font-medium"
                                style="color:var(--ink)">{{ $accessory->category ?? '—' }}</span>
                        </div>
                        <div class="flex items-center px-5 py-3" style="border-color:var(--line)">
                            ...
                            <span class="text-sm font-semibold font-mono tabular-nums"
                                ...
                                @if ($accessory->stock_qty <= 3 && $accessory->stock_qty > 0)
                                    <span class="ml-1.5 text-[11px] font-medium" style="color:var(--warn)">— Stok
                                        menipis</span>
                        ...
                        <div class="flex items-center px-5 py-3" style="border-color:var(--line)">
                            <span class="w-36 text-xs font-medium flex-shrink-0" style="color:var(--ink-mute)">Tanggal
                                Input</span>
                            <span class="text-sm font-mono"
                                style="color:var(--ink)">{{ $accessory->created_at->format('d M Y, H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border overflow-hidden" style="border-color:var(--line)">
                    <div class="px-5 py-3.5 flex items-center justify-between"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[11px] font-medium uppercase tracking-widest font-mono"
                                style="color:var(--ink-mute)">Rekap Pembayaran Penjualan</span>
                        </div>
                        @if ($saleHistory->count() > 0)
                            ...
                            @endphp
                            <div class="flex items-center gap-3">
                                <span class="text-[11px] font-mono" style="color:var(--ink-mute)">{{ $totalQtySold }}
                                    terjual</span>
                                <span class="text-[11px] font-bold font-mono tabular-nums" style="color:var(--success)">Rp
                                ...
                                <thead>
                                    <tr style="background:var(--bg-soft);border-bottom:1px solid var(--line)">
                                        <th class="text-left px-5 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Invoice</th>
                                        <th class="text-left px-4 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Tanggal</th>
                                        <th class="text-left px-4 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Pembeli</th>
                                        <th class="text-center px-4 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Qty</th>
                                        <th class="text-right px-4 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Subtotal</th>
                                        <th class="text-left px-5 py-2.5 font-bold uppercase tracking-wider font-mono"
                                            style="color:var(--ink-mute)">Pembayaran</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($saleHistory as $item)
                                        <tr style="border-bottom:1px solid var(--line)"
                                            onmouseenter="this.style.background='var(--bg-soft)'"
                                            onmouseleave="this.style.background=''">
                                            <td class="px-5 py-3 font-mono font-medium" style="color:var(--ink)">
                                                <a href="{{ route('sales.show', $item->sale) }}"
                                                    style="color:var(--accent)" class="hover:underline">
                                                    {{ $item->sale->invoice_number }}
                                                </a>
                                            </td>
                                            <td class="px-4 py-3 font-mono" style="color:var(--ink-soft)">
                                                {{ $item->sale->sale_date->format('d/m/Y') }}</td>
                                            ...
                                            <td class="px-4 py-3 text-center font-mono font-semibold"
                                                style="color:var(--ink)">{{ $item->quantity }}</td>
